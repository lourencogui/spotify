...
export function* pause() {
  try {
    ...
    console.log.tron('erro ao pausar a musica');
  }
}
export function* setSong(action) {
  try {
    ...
    yield put(PlayerActions.setSongFailure('Erro ao tocar a musica'));
  }
}
...
export function* next() {
  try {
    ...
    if (nextSong) yield put(PlayerActions.setSongRequest(nextSong, player.list));
  } catch (error) {
...
export function* previous() {
  try {
    ...
    if (previousSong) yield put(PlayerActions.setSongRequest(previousSong, player.list));
  } catch (error) {
    console.tron.log(error);
  }
}
